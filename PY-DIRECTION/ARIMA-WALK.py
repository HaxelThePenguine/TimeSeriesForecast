...
# Implementazioene ARIMA Walk-Forward per predizione direzionale
...
# funzione esercizio di stile tanto bast selzionare d=1 e trasforma da solo in stazionaria
"""
...
def auto_arima(series):
    ...
    try:
        ...
        try:
            result = adfuller(series.dropna(), autolag='AIC', maxlag=5) # uso lui per determinare d
        ...
        for order in candidate_orders:
            try:
                # molto automatico qui gli dai gli ordini e lui prova a fitare e vedere l'AIC
                ...
                if not np.isnan(aic) and aic < best_aic: # controllo nan e miglior aic
                    best_aic = aic
                    best_order = order
...
def train_arima_predict_direction(window_data, auto_order=True):
    ...
    try:
        ...
        # semplicemente arima da predizione di prezzo io ci calcolo la direzione dal rendimento se positivo o neg 
