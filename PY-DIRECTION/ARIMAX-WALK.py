...
## HELPER FUNCTIONS
## possibile implementare anche qua la selezione automatica delle exog
...
def create_walkforward_visualizations(results_df):
    """
    Crea visualizzazioni per i risultati walk-forward ARIMAX
    """
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))

    axes[0,0].plot(results_df['date'], results_df['cumulative_accuracy'], linewidth=2, color='blue', label='ARIMAX Walk-Forward')
    axes[0,0].axhline(y=50, color='red', linestyle='--', alpha=0.7, label='Random (50%)')
    axes[0,0].set_title('Directional Accuracy Cumulativa (ARIMAX)')
    ...
    for _, row in results_df.iterrows():
        order_str = str(row['arima_order'])
        if order_str not in order_performance:
            order_performance[order_str] = {'correct': 0, 'total': 0}
        order_performance[order_str]['total'] += 1
        if (row['predicted_direction'] == row['actual_direction']):
            order_performance[order_str]['correct'] += 1

    order_acc = {order: (perf['correct']/perf['total'])*100
                for order, perf in order_performance.items() if perf['total'] >= 5}

    if order_acc:
        top_orders = sorted(order_acc.items(), key=lambda x: x[1], reverse=True)[:5]
        orders, accuracies = zip(*top_orders)

        bars = axes[0,2].bar(range(len(orders)), accuracies, alpha=0.7, color='orange', edgecolor='black')
        axes[0,2].set_title('Accuracy per Ordine ARIMAX (Top 5)')
        axes[0,2].set_ylabel('Accuracy (%)')
        axes[0,2].set_xticks(range(len(orders)))
        axes[0,2].set_xticklabels([str(o) for o in orders], rotation=45)
    ...
    plt.tight_layout()
    plt.savefig('RESULTS/ARIMAX_WalkForward_Analysis.png', dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Grafici salvati in: RESULTS/ARIMAX_WalkForward_Analysis.png")

def save_walkforward_results(results_df):
    """
    Salva i risultati walk-forward ARIMAX in CSV
    """
    results_df.to_csv('RESULTS/ARIMAX_WalkForward_Results.csv', index=False)
    print(f"Risultati ARIMAX Walk-Forward salvati in: RESULTS/ARIMAX_WalkForward_Results.csv")
