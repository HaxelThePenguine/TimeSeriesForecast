...
def analyze_time_series(data):
    ...
    """
    print("ANALISI ESPLORATIVA SERIE STORICA VOLI EUROPEI (ETS)")
    print("="*60)
    ...
    if len(data) > 12:
        ...
        print(f"Stagionalità: {seasonality_strength:.1f}% variazione")
    ...
    if len(data) > 7:
        daily_avg = data.groupby(data.index.dayofweek)['Close'].mean()
        weekday_names = ['Lun', 'Mar', 'Mer', 'Gio', 'Ven', 'Sab', 'Dom']
        peak_day = daily_avg.idxmax()
        low_day = daily_avg.idxmin()
        print(f"Picco settimanale: {weekday_names[peak_day]} ({daily_avg[peak_day]:.0f} voli)")
        print(f"Minimo settimanale: {weekday_names[low_day]} ({daily_avg[low_day]:.0f} voli)")
    ...
    return {
        ...
        'seasonality_strength': seasonality_strength if 'seasonality_strength' in locals() else 0
    }
...
def visualize_ets_results(data, forecast_result, model_info, metrics):
    """
    Visualizzazione risultati ETS
    """
    ...
    fig.suptitle(f'ETS Analysis - {model_info["name"]} - Voli Europei 2016+ (con Close-up Previsioni)',
    ...
    if 'test_data' in metrics and 'test_forecasts' in metrics:
        ...
        axes[0,0].plot(test_dates, metrics['test_forecasts'], 'orange', linewidth=2,
                       linestyle='--', label='Previsioni ETS', marker='s', markersize=3)

        axes[0,0].fill_between(test_dates, metrics['test_data'], metrics['test_forecasts'],
                              color='red', alpha=0.3, label='Errore Previsione')
    else:
        axes[0,0].plot(forecast_dates, forecast_values, 'g-', linewidth=2, label='Previsioni ETS')
        axes[0,0].fill_between(forecast_dates, conf_int['lower'], conf_int['upper'],
                              color='green', alpha=0.2, label='95% CI (appross.)')
    ...
    axes[0,0].set_title(f'ETS {model_info["name"]} - Previsioni vs Test Set', fontweight='bold')
    ...
    axes[0,1].set_title('Residui del Modello ETS', fontweight='bold')
    ...
    if 'test_data' in metrics and 'test_forecasts' in metrics:
        ...
        axes[1,0].plot(test_dates, test_pred, 'orange', linewidth=2,
                       linestyle='--', label='Previsioni ETS', marker='s', markersize=5)
        ...
        axes[1,0].set_title('CLOSE-UP: Previsioni ETS vs Test Set', fontweight='bold', fontsize=12)
        ...
        if len(data) >= 14:
            try:
                decomposition = seasonal_decompose(data['Close'], model='additive', period=7, extrapolate_trend='freq')
                axes[1,0].plot(data.index, decomposition.seasonal, 'g-', linewidth=1)
                axes[1,0].set_title('Componente Stagionale (periodo 7)', fontweight='bold')
    ...
    if 'test_data' in metrics and 'test_forecasts' in metrics:
        ...
        axes[1,1].set_title('Errori Giornalieri ETS (Reale - Previsto)', fontweight='bold')
    ...
    residual_std = forecast_result['residual_std']
    ...
    if 'test_r2' in metrics:
        ...
        metrics_text = f"""METRICHE MODELLO ETS
{model_info['name']}
{'='*35}
...
DIAGNOSTICHE:
AIC: {aic_val:.2f}
Residual Std: {residual_std:.0f} voli
        """
    ...
    axes[2,0].set_title('ACF Residui ETS', fontweight='bold')
    ...
    axes[2,1].set_title('Distribuzione Residui ETS', fontweight='bold')
    ...
    if 'test_data' in metrics and 'test_forecasts' in metrics:
        ...
        axes[2,2].set_title('MAPE Cumulativo ETS per Orizzonte', fontweight='bold')
    ...
    plt.savefig('RESULTS/ETS_European_Flights_Analysis.png', dpi=300, bbox_inches='tight')
    plt.show()

    return fig

def save_results_to_csv(data, forecast_result, metrics, model_info):
    """
    ...
    historical_data = pd.DataFrame({
        'date': data.index[:len(fitted_values)],
        'actual_value': data['Close'].iloc[:len(fitted_values)],
    ...
    conf_int = forecast_result['confidence_intervals']
    results_df.loc[results_df['type'] == 'forecast', 'conf_lower'] = conf_int['lower'].values
    results_df.loc[results_df['type'] == 'forecast', 'conf_upper'] = conf_int['upper'].values

    import os
    os.makedirs('RESULTS', exist_ok=True)
    results_df.to_csv('RESULTS/ETS_European_Flights_Results.csv', index=False)
    print(f"Risultati salvati: RESULTS/ETS_European_Flights_Results.csv")
...
def main():
    ...
    save_results_to_csv(data, forecast_result, metrics, model_info)
    ...
    print(f"Grafici salvati: RESULTS/ETS_European_Flights_Analysis.png")
    print(f"Dati salvati: RESULTS/ETS_European_Flights_Results.csv")
    print(f"{'='*60}")
